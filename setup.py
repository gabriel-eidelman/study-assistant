...
def download_file(url, destination, is_json=False):
...
def download_face_models():
    """Download face-api.js model files from reliable sources."""
    # Create the static/models directory
    base_dir = Path("static/models")
    create_directory_if_not_exists(base_dir)
    
    # Model files needed for face-api.js - using reliable CDN sources
    model_files = [
    ...
    for file_info in model_files:
        ...
        is_json = file_info['is_json']
        destination = base_dir / filename
        
        if not download_file(url, destination, is_json):
            success = False
            print(f"WARNING: Failed to download {filename}. Face detection may not work correctly.")
            
            # Try alternative sources
            fallback_successful = try_alternative_source(file_info, destination)
            if fallback_successful:
                print(f"Fallback download successful for {filename}")
                success = True
    
    return success

def try_alternative_source(file_info, destination):
    """Try alternative sources for model files."""
    filename = file_info['filename']
    is_json = file_info['is_json']
    
    # Define alternative sources
    alt_sources = [
        f"https://raw.githubusercontent.com/justadudewhohacks/face-api.js/master/weights/{filename}",
        f"https://cdn.jsdelivr.net/npm/@vladmandic/face-api@1.6.10/model/{filename}"
    ]
    
    for alt_url in alt_sources:
        print(f"Trying alternative source: {alt_url}")
        if download_file(alt_url, destination, is_json):
            return True
    
    return False

def copy_client_file():
    """Copy the fixed client HTML file to the appropriate location."""
    print("Copying fixed test_client.html")
    
    if os.path.exists("fixed_test_client.html"):
        print("Backing up existing test_client.html if it exists")
        if os.path.exists("test_client.html"):
            backup_path = "test_client.html.backup"
            if os.path.exists(backup_path):
                os.remove(backup_path)  # Remove existing backup
            shutil.copy("test_client.html", backup_path)
        
        print("Updating test_client.html with fixed version")
        shutil.copy("fixed_test_client.html", "test_client.html")
    else:
        print("WARNING: fixed_test_client.html not found! Creating it now.")
        # Write the fixed content to the file
        with open("fixed_test_client.html", "w", encoding="utf-8") as f:
...
<body>
    ...
    <script>
        ...
        async function sendClientMetrics() {
            ...
            try {
                ...
                const clientMetrics = {
                    ...
                    seconds_since_last_movement: (now - lastBlinkTime) / 1000,
                    client_estimated_attention: attentionEstimate
                };
                
                console.log("Sending client metrics:", clientMetrics);
        ...
        # Copy the fixed file to test_client.html
        shutil.copy("fixed_test_client.html", "test_client.html")
        print("Created new fixed test_client.html file")

    # Copy api-test-page.html to the static directory if it exists
    if os.path.exists("api-test-page.html"):
        print("Copying api-test-page.html to static directory")
        create_directory_if_not_exists("static")
        shutil.copy("api-test-page.html", "static/api-test-page.html")
...
def create_sample_env_file():
    ...
    if not os.path.exists(".env"):
        ...
        with open(".env", "w") as f:
            f.write("# Azure Face API Credentials\n")
            f.write("AZURE_FACE_KEY=your_face_api_key_here\n")
            f.write("AZURE_FACE_ENDPOINT=https://your-resource-name.cognitiveservices.azure.com/\n")
        print("Created sample .env file. Please edit it with your Azure Face API credentials.")
...
def check_dependencies():
    ...
    try:
        ...
        # Add additional packages we need
...
def verify_models():
    """Verify if model files exist and are accessible."""
...
def main():
    ...
    print("==================================================")
    print("      Study Assistant Setup - FIXED VERSION       ")
    print("==================================================")
    ...
    model_success = download_face_models()
    if not model_success:
        print("\nWARNING: Some face detection models failed to download.")
    ...
    if verify_models():
        ...
        print("WARNING: Some model files are missing. Face detection may not work properly.")
    ...
    print("==================================================")
    print("1. Edit the .env file with your Azure Face API credentials")
    print("2. Start the application:")
    print("   python app.py")
    print("3. Visit http://localhost:8000/client in your browser")
    print("\nIf you still encounter issues with face detection:")
    print("- Check the JavaScript console for specific errors")
    print("- Make sure 'static/models' directory contains all model files")
    print("- Try using a different browser (Chrome recommended)")
    print("==================================================")
